...
import com.example.demo.repositories.mappers.EmployeeRowMapper;
...
@Autowired
    ...
    public List<Employee> getCashiersWithNumberOfChecks(UUID id) {
        // Updated SQL query with positional parameter
        String sql = "SELECT " +
                "    e.id_employee, " +
                "    e.empl_surname, " +
                "    e.empl_name, " +
                ...
                "    SUM(s.product_number) AS total_sold_products " +
                "FROM " +
                "    public.\"Employee\" e " +
                "LEFT JOIN " +
                "    public.\"Check\" c ON e.id_employee = c.id_employee " +
                "LEFT JOIN " +
                "    public.\"Sale\" s ON c.check_number = s.check_number " +
                "WHERE " +
                "    e.empl_role = 'Cashier' " +
                "    AND e.id_employee = ? " + // Use positional parameter
                "GROUP BY " +
                "    e.id_employee, e.empl_surname, e.empl_name, e.phone_number " +
                "ORDER BY " +
                "    number_of_checks DESC";

        List<Map<String, Object>> rows = namedJdbcTemplate.getJdbcTemplate().queryForList(sql, id);  // Pass `id` directly
        ...
        for (Map<String, Object> row : rows) {
            ...
            employee.setNumberOfChecks(numberOfChecks);  // Store the number of checks

            Long totalSoldProducts = (Long) row.get("total_sold_products");
            employee.setTotalAmountOfProducts(totalSoldProducts);  // Store the total sold products
