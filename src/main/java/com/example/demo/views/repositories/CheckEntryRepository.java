...
public class CheckEntryRepository{
    ...
    public List<CheckEntry> findAll() {
        String sql = "SELECT s.*, sp.selling_price, p.product_name\n" +
                "FROM \"Sale\" s\n" +
    ...
    public int delete(UUID check, UUID store_product) {
        String sql = "DELETE FROM public.\"Sale\"\n" +
                "\tWHERE \"UPC\" = ? AND check_number = ?;";
        return namedJdbcTemplate.getJdbcTemplate().update(sql, store_product, check);
    }
    public int count() {
